...
def fmt_time(sec):
    ...
    m = int(sec // 60)
    s = sec - m * 60
    return f"{m}:{s:06.3f}" if m else f"{s:0.3f}"
...
def current():
    try:
        ...
        if session_key is None or meeting_key is None:
            return jsonify({"ok": True, "meeting": "OpenF1", "session": "No active session", "rows": []})
        ...
        for d in drivers if isinstance(drivers, list) else []:
            ...
            if num:
                name_by_num[num] = d.get("broadcast_name") or d.get("full_name") or d.get("driver_name") or f"#{num}"
                team_by_num[num] = d.get("team_name") or "Reserve"

        best = {}
        for lap in laps if isinstance(laps, list) else []:
            num = str(lap.get("driver_number", ""))
            raw = lap.get("lap_duration") or lap.get("duration") or lap.get("lap_time")
            try:
                sec = float(raw)
            except Exception:
                continue
            if not num:
                continue
            if num not in best or sec < best[num]:
                best[num] = sec
        ...
        if rows:
            ...
            for i, r in enumerate(rows, start=1):
                r["pos"] = i
                gap = max(0.0, r["time"] - leader)
                r["gap"] = gap
                r["gap_text"] = "+0.000" if i == 1 else f"+{gap:0.3f}"
